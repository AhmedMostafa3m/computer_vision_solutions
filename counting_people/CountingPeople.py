import sys
sys.path.append(r'D:\Programs & tools\cmder')
...
cap = cv2.VideoCapture("04-people_counting\people.mp4")
...
mask = cv2.imread("04-people_counting/mask.png")

tracker = Sort(max_age = 20, min_hits = 3, iou_threshold = 0.3)

limitsDown = [440, 200, 840, 200]
limitsUp = [10, 350, 370, 350]
...
while True:
    ...
    imgGraphics = cv2.imread("04-people_counting/graphics.png", cv2.IMREAD_UNCHANGED)
    ...
    for result in trackerResults:
        ...
        cvzone.putTextRect(img, f'{int(id)}', (x1, y1-5), scale = 0.6, thickness = 1, offset=3)

        cx, cy = x1 + w // 2, y1 + h // 2
        cv2.circle(img, (cx, cy), 3, (255, 0, 0), cv2.FILLED)

        if limitsDown[0] < cx < limitsDown[2] and limitsDown[1] - 30 < cy < limitsDown[3] + 30:
